...
const auto createDeviceQueue = [](
	...
	auto pDevice = myvk::Device::Create(pPhysicalDevice,
	                                    myvk::GenericQueueSelector{&pQueue},
	                                    features, {VK_EXT_FRAGMENT_SHADER_INTERLOCK_EXTENSION_NAME});
	return {pDevice, pQueue};
};
}

Rasterizer::Rasterizer(uint32_t physicalDeviceID) {
	std::tie(mpDevice, mpQueue) = createDeviceQueue(
		[&](const std::vector<myvk::Ptr<myvk::PhysicalDevice>> &pPhysicalDevices) -> myvk::Ptr<myvk::PhysicalDevice> {
			...
			for (const auto &pCandidate : pPhysicalDevices) {
				if (pCandidate->GetExtensionSupport(VK_EXT_FRAGMENT_SHADER_INTERLOCK_EXTENSION_NAME) && pCandidate->
				    GetProperties().vk10.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU) {
					return pCandidate;
				}
			}
